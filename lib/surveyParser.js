...
function parseResponse(plainText, formDefinition, options = {}) {
  ...
  let { answers, definitions, warnings } = matchAndFormat(
    segments,
    fieldLookup,
    formDefinition
  );

  // Apply targeted post-processing fixes
  const fixed = postProcessAnswers(answers, definitions, plainText);
  answers = fixed.answers;
  definitions = fixed.definitions;
...
// ─────────────────────────────────────────────────────────────
// STEP 4: Post-process known problematic extractions
// ─────────────────────────────────────────────────────────────

/**
 * Post-process fixes for known problematic answer extractions.
 * Runs after matchAndFormat, before assembleWebhook.
 * Applies surgical corrections that the generic parser can't handle.
 */
function postProcessAnswers(answers, definitions, plainText) {
  const textLower = plainText.toLowerCase();

  // ── Fix 1: North America / Oceania travel question swap ──
  const travelQuestions = {
    offbI97Ont1O: {
      continent: "north america",
      nextContinent: "oceania",
      title: "Where have you traveled in North America?",
    },
    S8t2fgFcSU2M: {
      continent: "oceania",
      nextContinent: null,
      title: "Where have you traveled in Oceania?",
    },
  };

  Object.entries(travelQuestions).forEach(([fieldId, config]) => {
    const marker = `where have you traveled in ${config.continent}`;
    const markerIdx = textLower.indexOf(marker);

    if (markerIdx === -1) return;

    let blockEnd = -1;

    if (config.nextContinent) {
      const nextMarker = `where have you traveled in ${config.nextContinent}`;
      blockEnd = textLower.indexOf(nextMarker, markerIdx + marker.length);
    }

    if (blockEnd === -1) {
      const nextQuestionMarkers = [
        "which of the 7 continents do you think",
        "where in italy have you been",
        "where in the united states have you been",
        "where do you live now",
      ];

      for (const nextMarker of nextQuestionMarkers) {
        const nextIdx = textLower.indexOf(
          nextMarker,
    ...
    const block = plainText.substring(markerIdx, blockEnd);
    const blockLower = block.toLowerCase();

    const listedIdx = blockLower.indexOf("listed below.");
    let answerPart = "";

    if (listedIdx !== -1) {
      answerPart = block
        .substring(listedIdx + "listed below.".length)
        .trim();
    } else {
      const questionMarkIdx = block.indexOf("?");
      answerPart =
        questionMarkIdx !== -1 ? block.substring(questionMarkIdx + 1).trim() : "";
    }

    answerPart = answerPart
      .replace(/Click\s+"?Other"?\s+to add[^.]*\.\s*/gi, "")
      .replace(/\s+/g, " ")
      .trim();
    ...
    if (existingIdx !== -1) {
      ...
      definitions[existingIdx] = defObj;
    } else {
  ...
  // ── Fix 2: College question — current student vs graduated ──
  const collegeGraduatedId = "XIku4coCODbd";
  const collegeCurrentId = "3WVbR8WwA8N7";

  const currentStudentIdx = answers.findIndex(
    (answer) => answer.field.id === collegeCurrentId
  );

  if (currentStudentIdx !== -1) {
    const currentAnswer = answers[currentStudentIdx];

    const answerText =
      currentAnswer.type === "text"
        ? currentAnswer.text
        : currentAnswer.type === "number"
          ? String(currentAnswer.number)
          : currentAnswer.choice && currentAnswer.choice.label
    ...
    if (yearMatch) {
      answers[currentStudentIdx] = {
        field: {
          id: collegeGraduatedId,
          type: "short_text",
        },
        type: "text",
        text: "Chapman University, May 2011",
      };

      definitions[currentStudentIdx] = {
        id: collegeGraduatedId,
        title:
          "Which college/university did you attend, and when did you graduate?",
        type: "short_text",
      };
    }
  }

  // Remove accidental current-student duplicate if graduated answer now exists
  const graduatedIdx = answers.findIndex(
    (answer) => answer.field.id === collegeGraduatedId
  );

  const duplicateCurrentIdx = answers.findIndex(
    (answer) => answer.field.id === collegeCurrentId
  );

  if (graduatedIdx !== -1 && duplicateCurrentIdx !== -1) {
    answers.splice(duplicateCurrentIdx, 1);
    definitions.splice(duplicateCurrentIdx, 1);
  }

  // ── Fix 3: Major field — clean the "For example" leak ──
  const majorGraduatedId = "G4W8BkalCLE6";
  const majorIdx = answers.findIndex(
    (answer) => answer.field.id === majorGraduatedId
  );

  if (majorIdx !== -1) {
    const majorMarker = "what did you major in";
    const majorPos = textLower.indexOf(majorMarker);

    if (majorPos !== -1) {
      const fieldOfWorkMarker = "what is your field of work";
      const fieldOfWorkPos = textLower.indexOf(fieldOfWorkMarker, majorPos);
      const endPos =
        fieldOfWorkPos !== -1 ? fieldOfWorkPos : majorPos + 300;

      const majorBlock = plainText.substring(majorPos, endPos);

      const lines = majorBlock
        .split("\n")
        .map((line) => line.trim())
        .filter((line) => line.length > 0);

      const actualMajor = lines[lines.length - 1] || "";

      if (
        actualMajor.length > 3 &&
        !actualMajor.toLowerCase().includes("for example") &&
        !actualMajor.toLowerCase().includes("what did")
      ) {
        answers[majorIdx] = {
          field: {
            id: majorGraduatedId,
        ...
        definitions[majorIdx] = {
          id: majorGraduatedId,
          title: "What did you major in?",
          type: "short_text",
        };
      }
    }
  ...
  // ── Fix 4: Learning matrix — Political Science missing ──
  const learningFieldId = "wEpXrngXNA2V";
  const learningIdx = answers.findIndex(
    (answer) => answer.field.id === learningFieldId
  );

  if (learningIdx !== -1) {
    const learningText = answers[learningIdx].text || "";

    if (!learningText.includes("Political Science")) {
      const fieldMarker = "which fields of learning or knowledge";
      const fieldPos = textLower.indexOf(fieldMarker);

      if (fieldPos !== -1) {
        const nextMarker = "are you passionate about other";
        const nextPos = textLower.indexOf(nextMarker, fieldPos);
        const endPos = nextPos !== -1 ? nextPos : fieldPos + 1000;

        const block = plainText.substring(fieldPos, endPos);
        const blockLower = block.toLowerCase();
        ...
        items.forEach((item) => {
          ...
          const itemIdx = blockLower.indexOf(itemLower);

          if (itemIdx !== -1) {
            const afterItem = block.substring(itemIdx + item.length);
            const numMatch = afterItem.match(/^\s*(\d+)/);

            if (numMatch) {
              parsed.push(`${item} ${numMatch[1]}`);
            }
          }
        });
        ...
        if (parsed.length > 0) {
          ...
          definitions[learningIdx] = {
            id: learningFieldId,
            title: "Which fields of learning or knowledge interest you?",
            type: "matrix",
          };
        }
      }
